...
def extract_subtitle_segment(
    ...
    OVERLAP_THRESHOLD = 0.5  # 重叠比例阈值：超过50%才保留
    ...
    for sub in subtitles:
        ...
        # 字幕与剪辑区间部分重叠：
        # 计算重叠比例，超过阈值则保留并将时间戳裁剪到区间内，
        # 否则丢弃，避免出现过短或零时长的字幕残片
            ...
            if sub_duration <= 0 or overlap_duration / sub_duration <= OVERLAP_THRESHOLD:
                continue
